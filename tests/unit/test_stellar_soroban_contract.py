...
class TestContractSimulation:
    ...
    async def test_contract_simulation_accuracy(self, mock_contract_manager, mock_soroban_server):
        ...
        simulation_response = {
            ...
            "state_changes": [
                {"key": "balance_alice", "old_value": "1000", "new_value": "900"}
            ],
            "events": [
                {"type": "swap", "data": {"amount": "100", "token": "USDC"}}
            ]
        }
    ...
    async def test_simulation_failure_handling(self, mock_contract_manager, mock_soroban_server):
        ...
        assert simulation["success"] == False
...
class TestGasEstimation:
    ...
    async def test_gas_estimation_complexity_scaling(self, mock_contract_manager):
        ...
        simple_estimate = await mock_contract_manager.estimate_gas(
            "CONTRACT123", "simple_swap", simple_params
        )
        
        complex_estimate = await mock_contract_manager.estimate_gas(
            "CONTRACT123", "complex_arbitrage", complex_params
        )
    ...
    async def test_gas_estimation_network_conditions(self, mock_contract_manager):
        ...
        estimate = await mock_contract_manager.estimate_gas(
            "CONTRACT123", "swap", {"amount": "1000"}
        )
