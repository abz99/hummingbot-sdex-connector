...
class TestOptimalPathFinding:
    ...
    def sample_assets(self):
        ...
        return {
            "XLM": Asset.native(),
            "USDC": Asset("USDC", "GCKFBEIYTKP5RNAHAEUFLMIDKN5YFIF53MWKQK6VUD5XHAVTVL7LFLBV"),
    ...
    async def test_optimal_path_finding_direct(self, mock_path_engine, sample_assets):
        ...
        direct_route = await mock_find_direct_path(
            sample_assets["XLM"], 
            sample_assets["USDC"],
            Decimal("1000")
        )
...
class TestMEVProtection:
    ...
    async def test_mev_protection_private_mempool(self, mock_path_engine):
        ...
        async def mock_apply_mev_protection(transaction_id, route):
            ...
            return {
                "transaction_id": f"protected_{transaction_id}",
                **protection
            }
        ...
        assert protection_result["protected"] == True
    ...
    async def test_mev_protection_cost_benefit_analysis(self, mock_path_engine):
        ...
        for case in test_cases:
            is_worthwhile = calculate_protection_value(
                case["tx_amount"],
                case["mev_loss"],
                case["protection_cost"]
            )
