...
class TestRealWorldNetworkValidation:
    ...
    async def test_testnet_horizon_connectivity(self, testnet_config):
        ...
        try:
            ...
            assert ledger is not None, "Failed to fetch latest ledger"
            assert hasattr(ledger, 'sequence'), "Ledger missing sequence number"
            
            # Test network health
            network_health = await chain_interface.check_network_health()
            assert network_health, "Network health check failed"
    ...
    async def test_trustline_creation(self, funded_test_account, testnet_config):
        ...
        try:
            ...
            transaction_builder = await chain_interface.create_trustline_transaction(
                source_keypair=funded_test_account,
                asset=test_asset,
                limit="1000000"
            )
            ...
            for balance in balances:
                if (balance.get('asset_code') == test_asset.code and 
                    balance.get('asset_issuer') == test_asset.issuer):
    ...
    async def test_order_placement_on_testnet(self, stellar_connector, funded_test_account):
        ...
        try:
            ...
            if stellar_connector._security_framework:
                stellar_connector._security_framework._active_keypairs[funded_test_account.public_key] = funded_test_account
    ...
    async def test_secure_transaction_signing(self, funded_test_account, testnet_config):
        ...
        try:
            ...
            signed_message = await security_framework.sign_transaction(
                transaction_xdr=test_message,
                account_id=funded_test_account.public_key
            )
