...
from .stellar_logging import get_stellar_logger, LogCategory, with_correlation_id, get_request_logger
...
class StellarConnectionManager:
    ...
    def __init__(
        ...
        self._endpoint_health: Dict[str, Dict[str, float]] = {}  # service -> endpoint -> response_time
    ...
    def remove_endpoint_group(self, service_name: str):
        ...
        self.logger.info(
            f"Removed endpoint group: {service_name}",
            category=LogCategory.NETWORK
        )
    ...
    async def request(
        ...
        **kwargs: Any
        ...
        with with_correlation_id() as logger:
            # Log request start
            request_id = await self.request_logger.log_request(
                method, url, kwargs.get('headers')
            )
            ...
            try:
                ...
                async with session.request(method, url, **kwargs) as response:
                    ...
                    await self._update_request_metrics(
                        service_name, endpoint, True, response_time
                    )
                ...
                # Update metrics
                await self._update_request_metrics(
                    service_name, endpoint, False, response_time
                )
    ...
    async def _connection_cleanup_loop(self):
        ...
        while self._running:
            try:
                ...
                self.logger.error(
                    "Error in connection cleanup loop",
                    category=LogCategory.NETWORK,
                    exception=e
                )
    ...
    async def _on_connection_create(self, session, trace_config_ctx, params):
        ...
        self.logger.debug(
            f"Creating new connection to {params.host}:{params.port}",
            category=LogCategory.NETWORK
        )
    ...
    async def _on_connection_created(self, session, trace_config_ctx, params):
        ...
        self.logger.debug(
            f"Connection created to {params.host}:{params.port}",
            category=LogCategory.NETWORK
        )
