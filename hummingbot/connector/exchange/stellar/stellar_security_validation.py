...
def require_rate_limit(operation: str, scope: RateLimitScope = RateLimitScope.GLOBAL) -> Callable[[Callable], Callable]:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if rate_limiter:
                ...
                allowed = await rate_limiter.check_rate_limit(
                    operation, identifier, ip_address
                )
...
async def test_security_validation() -> bool:
    ...
    rate_limiter.configure_operation(
        'test_operation',
        RateLimitConfig(max_requests=3, time_window_seconds=10)
    )
