...
class StellarAssetVerifier:
            ...
            try:
                ...
                if not domain:
                    return self._create_failed_result(
                        asset_code, issuer, None, "No home domain found in issuer account"
                    )
                ...
                if not toml_content:
                    return self._create_failed_result(
                        asset_code, issuer, domain, f"Failed to fetch stellar.toml from {domain}"
                    )
                ...
                if not parsed_toml:
                    return self._create_failed_result(
                        asset_code, issuer, domain, "Failed to parse stellar.toml content"
                    )

                # Find asset in TOML
                asset_metadata = await self._find_asset_in_toml(parsed_toml, asset_code, issuer)

                if not asset_metadata:
                    return self._create_failed_result(
                        asset_code, issuer, domain, f"Asset {asset_code} not found in stellar.toml"
                    )
                ...
                self.metrics.record_network_request(
                    "asset_verification", "stellar_toml", "success", duration
                )
                ...
                # Record metrics
                self.metrics.record_network_request(
                    "asset_verification", "stellar_toml", "error", duration
                )

                return self._create_failed_result(
                    asset_code, issuer, None, f"Verification error: {str(e)}"
                )

    async def verify_multiple_assets(
        self, assets: List[Tuple[str, str]], max_concurrent: int = 10
    ...
    async def _parse_stellar_toml(self, content: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(
                "Failed to parse stellar.toml", category=LogCategory.SECURITY, exception=e
            )
            return None

    async def _find_asset_in_toml(
        self, parsed_toml: Dict[str, Any], asset_code: str, issuer: str
    ) -> Optional[AssetMetadata]:
        """Find asset information in parsed stellar.toml."""
        currencies = parsed_toml.get("CURRENCIES", [])

        for currency in currencies:
            if currency.get("code") == asset_code and currency.get("issuer") == issuer:
    ...
    async def _verify_with_directories(self, asset_code: str, issuer: str) -> Dict[str, Any]:
    ...
    async def _query_asset_directory(
        self, directory: AssetDirectory, asset_code: str, issuer: str
    ...
    def _create_failed_result(
        self, asset_code: str, issuer: str, domain: Optional[str], error_message: str
    ) -> VerificationResult:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        expired_entries = sum(
            1
            for result in self._verification_cache.values()
...
DEFAULT_ASSET_DIRECTORIES = [
    AssetDirectory(name="StellarTerm", base_url="https://stellarterm.com/directory/", priority=1),
    AssetDirectory(
        name="StellarExpert", base_url="https://stellar.expert/explorer/public/asset/", priority=2
    ),
    AssetDirectory(name="LOBSTR", base_url="https://lobstr.co/assets/", priority=3),
]
