...
class SecurityRequirementsTracker:
    ...
    def calculate_security_metrics(self) -> SecurityMetrics:
        ...
        critical_reqs = [r for r in self.requirements.values() if r.priority == RequirementPriority.P0_CRITICAL]
        high_reqs = [r for r in self.requirements.values() if r.priority == RequirementPriority.P1_HIGH]
        medium_reqs = [r for r in self.requirements.values() if r.priority == RequirementPriority.P2_MEDIUM]
        regulatory_reqs = [r for r in self.requirements.values() if r.priority == RequirementPriority.REG_REGULATORY]
        ...
        metrics = SecurityMetrics(
            ...
            compliance_score=(completion_rate(regulatory_reqs) + completion_rate(critical_reqs)) / 2,
    ...
    def get_requirements_by_priority(self, priority: RequirementPriority) -> List[SecurityRequirement]:
        ...
        for req in self.requirements.values():
            if req.status in [RequirementStatus.IN_PROGRESS, RequirementStatus.NOT_STARTED]:
                active_requirements.append({
                    ...
                    "owner": req.owner,
                    "due_date": datetime.fromtimestamp(req.due_date).strftime("%Y-%m-%d") if req.due_date else None,
...
|----|----------|-------|--------|-------|-------------|"""
        ...
        for req in active_reqs:
            ...
            due_date = datetime.fromtimestamp(req.due_date).strftime("%Y-%m-%d") if req.due_date else "TBD"
...
def update_security_requirement(req_id: str, status: RequirementStatus, 
                              completion_percentage: Optional[int] = None,
                              notes: str = "", user: str = "system"):
