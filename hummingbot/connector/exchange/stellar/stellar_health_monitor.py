...
class StellarHealthMonitor:
    ...
    def add_endpoint(self, url: str, check_type: HealthCheckType):
        ...
        if url not in self.endpoint_health:
            self.endpoint_health[url] = EndpointHealth(
                url=url,
    ...
    def remove_endpoint(self, url: str):
        ...
        if url in self.endpoint_health:
            ...
            self.logger.info(
                f"Removed endpoint from monitoring: {url}",
                category=LogCategory.HEALTH_CHECK
            )
    ...
    async def _monitoring_loop(self):
        ...
        while self._running:
            try:
                ...
                tasks = [
                    self.check_endpoint_health(url)
                    for url in self.endpoint_health.keys()
                ]
    ...
    async def _process_health_result(self, url: str, result: HealthCheckResult):
        ...
        if result.status == HealthStatus.HEALTHY:
            ...
            # Check for recovery
            if (old_status != HealthStatus.HEALTHY and 
                endpoint_health.consecutive_successes >= self.recovery_threshold):
    ...
    async def _handle_failure(self, url: str, result: HealthCheckResult):
        ...
        alert = Alert(
            ...
            severity=AlertSeverity.ERROR if endpoint_health.current_status == HealthStatus.DEGRADED else AlertSeverity.CRITICAL,
    ...
    def get_endpoint_status(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'success_rate': (health.total_checks - health.total_failures) / max(health.total_checks, 1),
