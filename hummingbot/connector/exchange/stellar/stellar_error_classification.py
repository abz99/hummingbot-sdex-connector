...
class StellarErrorClassifier:
    ...
    def _classify_unknown_error(self, error: Exception, context: Optional[ErrorContext]) -> ErrorClassification:
        ...
        if any(pattern in error_msg for pattern in ['unauthorized', 'authentication', '401', 'forbidden', '403']):
            return ErrorClassification(
...
class ErrorRecoveryHandler(ABC):
    ...
    @abstractmethod
    async def handle(self, error: Exception, classification: ErrorClassification, context: ErrorContext) -> bool:
        """
...
class RetryWithBackoffHandler(ErrorRecoveryHandler):
    ...
    async def can_handle(self, classification: ErrorClassification, context: ErrorContext) -> bool:
        return classification.recovery_strategy == RecoveryStrategy.RETRY_WITH_BACKOFF and classification.is_retryable
...
class RetryWithJitterHandler(ErrorRecoveryHandler):
    ...
    async def can_handle(self, classification: ErrorClassification, context: ErrorContext) -> bool:
        return classification.recovery_strategy == RecoveryStrategy.RETRY_WITH_JITTER and classification.is_retryable
