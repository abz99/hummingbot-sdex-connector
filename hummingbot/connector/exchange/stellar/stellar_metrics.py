...
class StellarMetrics:
    ...
    def create_custom_metric(self, definition: MetricDefinition) -> Any:
        ...
        if definition.metric_type == MetricType.COUNTER:
            ...
            metric = Histogram(definition.name, definition.description, definition.labels, registry=self.registry)
        elif definition.metric_type == MetricType.GAUGE:
            metric = Gauge(definition.name, definition.description, definition.labels, registry=self.registry)
        elif definition.metric_type == MetricType.SUMMARY:
            metric = Summary(definition.name, definition.description, definition.labels, registry=self.registry)
    ...
    def record_network_request(self, network: str, endpoint_type: str, status: str, duration: float):
        """Record a network request."""
        self.network_requests_total.labels(network=network, endpoint_type=endpoint_type, status=status).inc()
        self.network_request_duration.labels(network=network, endpoint_type=endpoint_type).observe(duration)
    
    def record_network_error(self, network: str, endpoint_type: str, error_type: str):
        """Record a network error."""
        self.network_errors_total.labels(network=network, endpoint_type=endpoint_type, error_type=error_type).inc()
    ...
    def record_order_placement(self, network: str, trading_pair: str, side: str, status: str):
        ...
        self.orders_placed_total.labels(network=network, trading_pair=trading_pair, side=side, status=status).inc()
    
    def record_order_fill(self, network: str, trading_pair: str, side: str, duration: float):
        """Record an order fill."""
        self.order_fill_duration.labels(network=network, trading_pair=trading_pair, side=side).observe(duration)
    
    def record_trading_volume(self, network: str, trading_pair: str, side: str, volume_xlm: float):
        """Record trading volume."""
        self.trading_volume.labels(network=network, trading_pair=trading_pair, side=side).inc(volume_xlm)
    
    def set_order_book_spread(self, network: str, trading_pair: str, spread_percent: float):
        """Set the current order book spread."""
        self.order_book_spread.labels(network=network, trading_pair=trading_pair).set(spread_percent)
    
    # Account metrics methods
    def set_account_balance(self, network: str, account_id: str, asset_code: str, balance_xlm: float):
        """Set account balance."""
        self.account_balances.labels(network=network, account_id=account_id, asset_code=asset_code).set(balance_xlm)
    ...
    def set_endpoint_health(self, network: str, endpoint_type: str, url: str, is_healthy: bool):
        ...
        self.endpoint_health_status.labels(network=network, endpoint_type=endpoint_type, url=url).set(1 if is_healthy else 0)
    ...
    def set_profit_loss(self, network: str, trading_pair: str, strategy: str, pnl_xlm: float):
        """Set profit/loss."""
        self.profit_loss_xlm.labels(network=network, trading_pair=trading_pair, strategy=strategy).set(pnl_xlm)
    
    def record_arbitrage_opportunity(self, network: str, asset_pair: str, executed: bool):
        """Record an arbitrage opportunity."""
        self.arbitrage_opportunities.labels(network=network, asset_pair=asset_pair, executed=str(executed).lower()).inc()
    ...
    async def stop_background_metrics_collection(self):
        ...
        self.logger.info(
            "Background metrics collection stopped",
            category=LogCategory.METRICS
        )
