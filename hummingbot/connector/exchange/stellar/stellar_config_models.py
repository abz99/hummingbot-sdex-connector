...
class StellarNetworkConfig(BaseModel):
    ...
    history_archives: List[HttpUrl] = Field(default_factory=list, description="History archive URLs")
    ...
    well_known_assets: Dict[str, AssetConfig] = Field(default_factory=dict, description="Well-known assets")
...
class PerformanceConfig(BaseModel):
    ...
    max_concurrent_requests: int = Field(default=50, ge=1, le=1000, description="Max concurrent requests")
...
class CacheConfig(BaseModel):
    ...
    account_info_ttl: int = Field(default=30, ge=1, le=3600, description="Account info TTL in seconds")
...
class AlertConfig(BaseModel):
    """Alert configuration."""
    network_down_threshold: int = Field(default=3, ge=1, le=100, description="Network down threshold")
    high_latency_threshold: int = Field(default=5000, ge=100, le=60000, description="High latency threshold in ms")
    error_rate_threshold: float = Field(default=0.1, ge=0.0, le=1.0, description="Error rate threshold (0-1)")
...
class AMMLiquidityConfig(BaseModel):
    ...
    min_liquidity: int = Field(default=1000, ge=1, description="Minimum liquidity in XLM equivalent")


class AMMConfig(BaseModel):
    """AMM configuration."""
    contracts: Dict[str, Dict[str, str]] = Field(default_factory=dict, description="AMM contracts by network")
    rewards: AMMLiquidityConfig = Field(default_factory=AMMLiquidityConfig, description="Liquidity rewards config")


class StellarMainConfig(BaseModel):
    """Main Stellar configuration with comprehensive validation."""
    default_network: StellarNetworkType = Field(default=StellarNetworkType.TESTNET, description="Default network")
    auto_failover: bool = Field(default=True, description="Enable automatic failover")
    health_check_interval: int = Field(default=30, ge=1, le=3600, description="Health check interval")
    ...
    networks: Dict[StellarNetworkType, StellarNetworkConfig] = Field(..., description="Network configurations")
    
    # Asset and trading configuration
    asset_directories: List[AssetDirectoryConfig] = Field(default_factory=list, description="Asset directories")
    well_known_assets: Dict[str, Dict[str, AssetConfig]] = Field(default_factory=dict, description="Well-known assets by network")
    trading_pairs: Dict[str, TradingPairsConfig] = Field(default_factory=dict, description="Trading pairs by network")
    ...
    circuit_breaker: CircuitBreakerConfig = Field(default_factory=CircuitBreakerConfig, description="Circuit breaker settings")
    ...
    def validate_default_network_exists(cls, v, values):
        ...
        if default_network and default_network not in v:
            raise ValueError(f"Default network '{default_network}' not found in networks configuration")
        return v
        ...
        for network_type, network_config in config.networks.items():
            ...
            if network_type == StellarNetworkType.MAINNET:
                if len(network_config.horizon.fallbacks) < 2:
                    warnings.append(f"Mainnet should have at least 2 fallback Horizon endpoints")
                
                if network_config.friendbot and network_config.friendbot.enabled:
                    warnings.append(f"Friendbot should not be enabled on mainnet")
            ...
            # Check for test network requirements
                ...
                if not network_config.friendbot or not network_config.friendbot.enabled:
                    warnings.append(f"{network_type.value} should have friendbot enabled for testing")
