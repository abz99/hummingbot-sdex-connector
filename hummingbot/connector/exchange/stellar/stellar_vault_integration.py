...
class VaultKeyStore(KeyStoreBackend):
        ...
        try:
            ...
            async with self._session.post(
                ...
                if response.status in [200, 204]:
                    ...
                    error_text = await response.text()
                    raise Exception(f"Failed to store key in Vault: {response.status} - {error_text}")
            ...
            async with self._session.get(
                ...
                if response.status == 200:
                    ...
                    metadata = KeyMetadata(
                        ...
                        security_level=getattr(SecurityLevel, meta_dict.get('security_level', 'PRODUCTION')),
                    ...
                    error_text = await response.text()
                    raise Exception(f"Failed to retrieve key from Vault: {response.status} - {error_text}")
        ...
        try:
            ...
            if self.config.engine_type == VaultEngine.KV_V2:
                ...
                async with self._session.delete(
                    f"{self.config.url}/v1/{metadata_path}",
                    headers=headers
                ) as response:
            ...
            if success:
                ...
                error_text = await response.text()
                raise Exception(f"Failed to delete key from Vault: {response.status} - {error_text}")
    ...
    async def list_keys(self) -> List[KeyMetadata]:
        ...
        try:
            ...
            async with self._session.request(
                'LIST',
                f"{self.config.url}/v1/{list_path}",
                headers=headers
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    ...
                    error_text = await response.text()
                    raise Exception(f"Failed to list keys in Vault: {response.status} - {error_text}")
                    
        except Exception as e:
            self.logger.error(
                f"Failed to list keys in Vault: {e}",
                category=LogCategory.SECURITY,
                exception=e
            )
            return []
    ...
    async def renew_token(self) -> bool:
        ...
        try:
            ...
            async with self._session.post(
                f"{self.config.url}/v1/auth/token/renew-self",
                headers=headers
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    ...
                    lease_duration = auth_info.get('lease_duration', 0)
                    self._token.expires_at = time.time() + lease_duration if lease_duration > 0 else float('inf')
            ...
            self.logger.error(
                f"Failed to renew Vault token: {e}",
                category=LogCategory.SECURITY,
                exception=e
            )
            return False
