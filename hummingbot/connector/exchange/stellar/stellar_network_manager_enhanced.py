...
class EnhancedStellarNetworkManager:
    ...
    def _load_and_validate_configuration(self) -> None:
        ...
        with with_correlation_id() as logger:
            ...
            try:
                ...
                warnings = self.config_validator.validate_network_connectivity(self.validated_config)
    ...
    async def _initialize_connection_manager(self) -> None:
        ...
        self.connection_manager = StellarConnectionManager(
            load_balance_strategy=LoadBalanceStrategy.RESPONSE_TIME,
            enable_http2=True
        )
        ...
        for network_type, network_config in self.validated_config.networks.items():
            ...
            self.connection_manager.add_endpoint_group(f"soroban_{network_type.value}", soroban_endpoints)
    ...
    async def _initialize_health_monitoring(self) -> None:
        ...
        for network_type, network_config in self.validated_config.networks.items():
            ...
            self.health_monitor.add_endpoint(
                str(network_config.horizon.primary),
                HealthCheckType.HORIZON_API
            )
            ...
            self.health_monitor.add_endpoint(
                str(network_config.soroban.primary),
                HealthCheckType.SOROBAN_RPC
            )
            ...
            if network_config.friendbot and network_config.friendbot.enabled:
                self.health_monitor.add_endpoint(
                    str(network_config.friendbot.url),
                    HealthCheckType.FRIENDBOT
                )
    ...
    async def _start_background_tasks(self) -> None:
        ...
        if self.metrics:
            self._background_tasks.append(
                asyncio.create_task(self._network_metrics_loop())
            )
    ...
    async def _on_endpoint_recovery(self, endpoint: str, result) -> None:
        ...
        self.logger.info(
            f"Endpoint recovery detected: {endpoint}",
            category=LogCategory.NETWORK
        )
    ...
    def _determine_network_from_endpoint(self, endpoint: str) -> Optional[StellarNetworkType]:
        ...
        for network_type, network_config in self.validated_config.networks.items():
            if (str(network_config.horizon.primary) == endpoint or
                endpoint in [str(url) for url in network_config.horizon.fallbacks] or
                str(network_config.soroban.primary) == endpoint or
                endpoint in [str(url) for url in network_config.soroban.fallbacks]):
                return network_type
        return None
    ...
    async def check_network_health(self, network: Optional[StellarNetworkType] = None) -> NetworkHealth:
    ...
    async def verify_asset(
        ...
        asset_code: str, 
        issuer: str, 
        network: Optional[StellarNetworkType] = None
    ):
        ...
        with with_correlation_id() as logger:
            ...
            try:
                ...
                if service_name not in self.connection_manager.endpoints:
                    friendbot_endpoints = [EndpointConfig(
                        url=str(network_config.friendbot.url),
                        priority=1
                    )]
                ...
                if success:
                    logger.info(
                        f"Test account funded successfully",
                        category=LogCategory.NETWORK,
                        account=public_key[:8] + "..."
                    )
                else:
                    logger.warning(
                        f"Test account funding failed",
                        category=LogCategory.NETWORK,
    ...
    def get_comprehensive_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            'configured_networks': list(self.validated_config.networks.keys()) if self.validated_config else [],
