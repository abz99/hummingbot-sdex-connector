...
def add_stellar_context(logger: WrappedLogger, method_name: str, event_dict: EventDict) -> EventDict:
...
class StellarLogger:
    ...
    def with_context(self, **context: Any) -> structlog.BoundLogger:
    ...
    def error(self, message: str, category: Optional[LogCategory] = None, exception: Optional[Exception] = None, **kwargs: Any) -> None:
        """Log error message."""
        if exception:
            kwargs['exception'] = exception
        self._log(LogLevel.ERROR, message, category, **kwargs)
    
    def critical(self, message: str, category: Optional[LogCategory] = None, exception: Optional[Exception] = None, **kwargs: Any) -> None:
        """Log critical message."""
        if exception:
            kwargs['exception'] = exception
        self._log(LogLevel.CRITICAL, message, category, **kwargs)
    
    def _log(self, level: LogLevel, message: str, category: Optional[LogCategory] = None, **kwargs: Any) -> None:
...
class RequestLogger:
    ...
    async def log_request(self, method: str, url: str, headers: Optional[Dict[str, str]] = None, **kwargs: Any) -> str:
    ...
    async def log_response(self, req_id: str, status_code: int, response_time: float, **kwargs: Any) -> None:
        """Log HTTP response."""
        with request_id.set(req_id):
...
class PerformanceLogger:
    ...
    async def log_operation_end(self, op_id: str, operation: str, duration: float, success: bool = True, **context: Any) -> None:
...
def log_error(message: str, exception: Optional[Exception] = None, category: Optional[LogCategory] = None, **kwargs: Any) -> None:
