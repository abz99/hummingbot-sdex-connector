...
class StellarNetworkManager:
    ...
    def _parse_network_config(self, name: str, data: Dict[str, Any]) -> StellarNetworkConfig:
        ...
        soroban_endpoints = [
            NetworkEndpoint(
                url=soroban_config.get('primary', ''),
                timeout=soroban_config.get('timeout', 20)
            )
        ]
        ...
        for network, config in self.network_configs.items():
            if config.horizon_endpoints:
                ...
                self.servers[network] = Server(
                    horizon_url=primary_endpoint.url,
                    client=self._session
                )
        
        # Start health monitoring
        if self.config.get('stellar', {}).get('monitoring', {}).get('health_checks', {}).get('enabled', True):
            interval = self.config.get('stellar', {}).get('monitoring', {}).get('health_checks', {}).get('interval', 30)
